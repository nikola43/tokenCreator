...
contract TetrisGame is Ownable, IERC721Receiver, IERC1155Receiver, ERC721Holder, ERC1155Holder {
    ...
    function getHighScoreAddressess(string memory name) public view returns(address[] memory) {
        require(games[name].isExist == true, "Error (storeScore): game not found");
        return games[name].rankingListAddressess;
    }

    function getHighScores(string memory name) public view returns(uint256[] memory) {
        require(games[name].isExist == true, "Error (storeScore): game not found");
    ...
    function setRewardedNFT(string memory name, uint nftType, address nftAddress, uint nftId) public onlyOwner {
        require(games[name].isExist == true, "Error (storeScore): game not found");
        games[name].rewardedNftType = nftType;
