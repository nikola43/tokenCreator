...
export class TokenGeneratorComponent implements OnInit {
          ...
          this.http.post('https://api-testnet.bscscan.com/api', formData).subscribe(
            async (response: any) => {
              ...
              if (response.status === '1') {
                ...
                this.tokenBalance = Number(Web3.utils.fromWei(await this.getTokenBalance(this.createdTokenAddress),
                  'ether')).toFixed(18).toString();
  ...
  async burnTokenInputKeyUp() {
    this.tokenBalance = Number(Web3.utils.fromWei(
    ...
    const value = this.mapValue(Number(this.burnTokenForm.amount), 0, this.tokenBalance, 0, 100);
  ...
  async tokenInputKeyUp() {
    ...
    if (isValid) {
      this.tokenBalance = Number(Web3.utils.fromWei(await this.getTokenBalance(
  ...
  async approveToken() {
    ...
    await this.web3Service.approveToken(tokenAddress, tokenAmount).then((r) => {
      if (r) {
  ...
  async burnTokens() {
    ...
    console.log(this.burnTokenAddressInputFormGroup.controls.burnTokenAddress.value);
    this.web3Service.burnTokens(
      this.burnTokenAddressInputFormGroup.controls.burnTokenAddress.value, this.burnTokenForm.amount).then((r2) => {
      console.log(r2);
    }).catch((err) => {
      console.log(err);
    });
  ...
  async addLiquidity() {
    ...
    this.web3Service.addLiquidity(this.tokenAddressInputFormGroup.controls.liquidityTokenAddress.value,
      bnbAmount, tokenAmount, minBnbTokenAmount, minTokenAmount).then(async (r2) => {
      const pairAddress = await this.getPair(this.createdTokenAddress);
      this.tokenBalance = Number(Web3.utils.fromWei(await this.getTokenBalance(this.createdTokenAddress), 'ether')).toFixed(18).toString();
      this.bnbBalance = Number(Web3.utils.fromWei(await this.web3Service.getBalance(), 'ether')).toFixed(18).toString();
      this.lpTokenBalance = Number(Web3.utils.fromWei(await this.getLPTokenBalance(pairAddress), 'ether'));

      this.lockLiquidityTokenAddressInputFormGroup.controls.lockLiquidityTokenAddress.setValue(pairAddress);
  ...
  async onLockLiquidityTokenAddressKeyup() {
    ...
    if (isValid) {
      this.lpTokenBalance = Number(Web3.utils.fromWei(await this.getLPTokenBalance(
